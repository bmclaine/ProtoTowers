...
void main(uint3 ThreadID : SV_DispatchThreadID)
{
	if (ThreadID.x < DeadCount && ThreadID.x < Emission)
	{
		...
		float2 randomScalar = normalize(RandomValuesTexture.SampleLevel(Sampler, uv, 0).xy);

		// Create a position from the base of the cone and the top of the cone using the random scalar value
		PartA.Position = WorldPosition + mul(Rotation, float4(ShapeDimensions.x * randomScalar.x, 0, ShapeDimensions.x * randomScalar.y, 0)).xyz;
		float3 ToPosition = WorldPosition + mul(Rotation, float4(ShapeDimensions.y * randomScalar.x, ShapeDimensions.z, ShapeDimensions.y * randomScalar.y, 0)).xyz;

		// Create our velocity for the particle
		PartA.Velocity = normalize(ToPosition - PartA.Position) * StartSpeed;
