...
//
// Interface that communicates with the modules of the engine, for purposes like registering and unregistering.
#pragma once
...
namespace InfernoEngine_Graphics
{
	...
	int CreateUnorderedAccessView(int textureID, unsigned int Flags, unsigned int format = 0);
	...
	int CreateSolidColorImage(std::wstring name, unsigned int width, unsigned int height, unsigned int borderSize, Color backColor = Color(1, 1, 1, 1), Color borderColor = Color(0, 0, 0, 1));
	...
	void GetSupportedResolutions(std::vector<std::wstring>& resolutions, unsigned int displayID = 0);
	bool VSyncEnabled();
...
namespace InfernoEngine_Input
{
	...
	//  Unregisters the given delegate function from the Action Binding, if it exists. Returns true if it was found and unregistered,
	//  false other wise.
	bool UnregisterEventFromActionBinding(unsigned int _hash, void* _objectObject, ActionEventFunction _delegate);
